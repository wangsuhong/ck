...
<html lang="zh">
	...
	<body>
		<div class="card">
			...
			<div class="card-body">
				...
				<form class="form-horizontal" action="ExpoServlet?method=expoUpdate" method="post" id="myform">
					<input type="hidden" class="form-control" name="id" id="id" value="${expo.id}" />
					<input type="hidden" class="form-control" name="oldgoodsid" id="oldgoodsid" value="${expo.goodsid}" />
					<label class="col-md-3 control-label">出库货物编号</label>
					<div class="col-xs-4">
						<input type="text" class="form-control" name="goodsid" id="goodsid" value="${expo.goodsid}" />
					</div>
					<div style="height: 50px;"><a id="goodsidMsg"></a></div>
					<label class="col-md-3 control-label">出库货物介绍</label>
					<div class="col-xs-4">
						<input type="text" class="form-control" name="exponame" id="exponame" value="${expo.exponame}"/>
					</div>
					<div style="height: 50px;"></div>
					<label class="col-md-3 control-label">出库货物数量</label>
					<div class="col-xs-4">
						<input type="text" class="form-control" name="exponum" id="exponum" value="${expo.exponum}" />
					</div>
					<div style="height: 50px;"><a id="exponumMsg"></a></div>
					<label class="col-md-3 control-label">出库货物单价</label>
					<div class="col-xs-4">
						<input type="text" class="form-control" name="expoprice" id="expoprice" value="${expo.expoprice}" />
					</div>
					<div style="height: 50px;"></div>
					<label class="col-md-3 control-label">出库货物登记时间</label>
					<div class="col-xs-4">
						<input class="form-control js-datetimepicker" type="text" id="expodate" name="expodate" value="${expo.expodate}"
	...
	$("#exponum").blur(function() {
		...
		checkExpoUp(id,exponum,goodsid);
	  })
	})
	function checkExpoUp(id,exponum,goodsid){
		$.ajax({
			url: "CheckServlet?method=checkExpoUp", //url
		    type : "post",
		    dataType: "text",
			data : {"id": id,"exponum": exponum,"goodsid": goodsid},
			success : function(s) {
			//提示数量
			    if(s == "1"){
				$("#exponumMsg").html("出库该数量后货物数量小于0!!!");
				}else if(s=="0"){
				$("#exponumMsg").html("修改后货物数量大于库存容量!!!");
				}else{
				$("#exponumMsg").html("");
				}
			  }
