...
<html lang="zh">
	...
	<body>
		<div class="card">
			...
			<div class="card-body">
				...
				<form class="form-horizontal" action="ImpoServlet?method=ImpoAdd" method="post" id="myform">
					...
					<div class="col-xs-4">
						<input type="text" class="form-control" name="imponame" id="imponame" value=""/>
					</div>
					<div style="height: 50px;"></div>
					<label class="col-md-3 control-label">入库货物数量</label>
					<div class="col-xs-4">
						<input type="text" class="form-control" name="imponum" id="imponum" value="" />
					</div>
					<div style="height: 50px;"><a id="imponumMsg"></a></div>
					<label class="col-md-3 control-label">入库货物单价</label>
					<div class="col-xs-4">
						<input type="text" class="form-control" name="impoprice" id="impoprice" value="" />
					</div>
					<div style="height: 50px;"></div>
					<label class="col-md-3 control-label">入库货物登记时间</label>
					<div class="col-xs-4">
						<input class="form-control js-datetimepicker" type="text" id="impodate" name="impodate" value=""
	...
	$("#imponum").blur(function() {
		...
		if(!r.test(imponum)){
			$("#imponumMsg").html("入库货物数量必须为数字!!!");
			return;
		}
		checkImpo(imponum,goodsid);
	  })
	})
	function checkImpo(imponum,goodsid){
		$.ajax({
			url: "CheckServlet?method=checkImpo", //url
		    type : "post",
		    dataType: "text",
			data : {"imponum": imponum,"goodsid": goodsid},
			success : function(s) {
			//提示数量
			    if(s == "1"){
				$("#imponumMsg").html("入库该货物后数量大于库存!!!");
				}else{
				$("#imponumMsg").html("");
				}
			  }
